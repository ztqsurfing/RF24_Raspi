...
//tq add
//for dsdv route protocol to read dv msg
...
//针对的是本地发出的报文
//根据传入的header和msg，构成发送帧
//根据传入的writeDirect参数，确定是正常发送--070 normal，还是广播--0100，还是直接向物理地址发送
//调用write函数将frame写出去
...
bool RF24Network::write(uint16_t to_node, uint8_t directTo)  // Direct To: 0 = First Payload, standard routing, 1=routed payload, 2=directRoute to host, 3=directRoute to Route
{
  ...
  #endif
  /**Write it*/
  //conversion中的send_node表示要发送到的节点，对应的send_pipe表示要发送到的节点的mac
  ok=write_to_pipe(conversion.send_node, conversion.send_pipe, conversion.multicast);  	
	...
	//如果发送的报文需要ack，而本节点成功的将报文直接发送给了目的节点，则需要向源节点发送一个ack消息
	...
	//如果发送的报文需要ack，而本节点又不是直接发送至目的节点，则需要等待下一跳返回ack
	if( ok && conversion.send_node != to_node && (directTo==0 || directTo==3) && isAckType){
		...
		//update中会对收到的ack进行路由返回，然后返回值表示收到了ack类型的报文
...
bool RF24Network::logicalToPhysicalAddress(logicalToPhysicalStruct *conversionInfo){
  ...
  uint16_t arg1,arg2;
	...
	while(!feof(fp)){
		...
		sprintf(log_print_tmp,"route entry:(target)%#o->(nxt node)%#o\n",arg1,arg2);
 ...
 if(*directTo > TX_ROUTED ){    
	...
	*multicast = 1;
	...
	if(header->type == DSDV_TYPE)
		...
		if(route_table.count(*to_node) != 0)
		{
			pre_conversion_send_node = route_table[*to_node];
			pre_conversion_send_pipe = 5;
		}
		else
			//目的不可达
			Rlog(ERROR,"***cannot send or relay this msg, no routing entry!");  
...
bool RF24Network::write_to_pipe( uint16_t node, uint8_t pipe, bool multicast )
{
  ...
  char log_print_tmp[30];
  sprintf(log_print_tmp,"Routing info: fr N <%u> to nxt N <%u> in pipe <%u>",node_address,node,out_pipe);
  Rlog(INFO,log_print_tmp);
